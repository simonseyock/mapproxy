...
from config.configuration.base import ConfigurationBase

from mapproxy.config.loader import ConfigurationError, parse_color
...
class SourcesCollection(dict):
    ...
    ``['source_name:lyr,lyr2']`` will return the source with ``source_name``
...
class SourceConfiguration(ConfigurationBase):
    ...
    def supported_srs(self):
        ...
        if not supported_srs:
            return None
        return SupportedSRS(supported_srs, config.configuration.global_configuration.preferred_srs)
